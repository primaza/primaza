...
@step(u'On Primaza Cluster "{primaza_cluster_name}", the status of Claim "{service_claim_name}" is "{status}"')
...
@step(u'On Primaza Cluster "{cluster_name}", the RegisteredService bound to the Claim "{claim_name}" is "{registered_service}"')
def ensure_registered_service_of_service_claim(context, cluster_name: str, claim_name: str, registered_service: str, tenant: str = "primaza-system"):
    cluster = context.cluster_provider.get_primaza_cluster(cluster_name)
    group = "primaza.io"
    version = "v1alpha1"
    plural = "claims"

    def check_claim_status() -> bool:
