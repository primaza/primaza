...
@step(u'On Worker Cluster "{cluster_name}", the status of ApplicationServiceClaim "{service_claim_name}" is "{status}"')
def ensure_status_of_service_claim(context, cluster_name: str, service_claim_name: str, status: str):
    ...
    group = "primaza.io"
    version = "v1alpha1"
    plural = "applicationserviceclaims"
...
@step(u'On Worker Cluster "{cluster_name}", the RegisteredService bound to the ApplicationServiceClaim "{service_claim_name}" is "{registered_service}"')
