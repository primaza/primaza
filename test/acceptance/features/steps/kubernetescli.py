...
@step(u'{resource_type}/{resource_name} is available in cluster "{cluster}"')
def check_resource_exists(context, resource_type, resource_name, cluster):
    resource_name = substitute_scenario_id(context, resource_name)
    ...
    with tempfile.NamedTemporaryFile() as tf:
        ...
        assert kubernetes.is_resource_in(resource_type, resource_name), \
...
@step(u'jsonpath "{expr}" on "{resource_type}/{name}" in cluster {cluster} is "{text}"')
def jq_is(context, expr, resource_type, name, cluster, text):
    if "namespace" in context:
        ns = context.namespace.name
    else:
        ns = None
    ...
    with tempfile.NamedTemporaryFile() as tf:
        ...
        kubernetes = Kubernetes(kubeconfig=tf.name)
        polling2.poll(lambda: kubernetes.get_resource_info_by_jq(resource_type, name, ns, expr),
                      step=1, timeout=400,
