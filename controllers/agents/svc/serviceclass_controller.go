...
//+kubebuilder:rbac:groups=apps/v1,resources=deployments,verbs=get;list;update;patch;watch
...
func (r *ServiceClassReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	reconcileLog.Info("Reconciling service class in agent service", "namespace", req.Namespace, "name", req.Name)
...
func (r *ServiceClassReconciler) setOwnerReference(ctx context.Context, scclass *v1alpha1.ServiceClass, namespace string) error {
	...
	if len(agentsvcdeployment.OwnerReferences) == 0 {
		if err := ctrl.SetControllerReference(&agentsvcdeployment, scclass, r.RemoteScheme); err != nil {
			return err
		}
	} else {
		var found bool
		for _, owner := range agentsvcdeployment.OwnerReferences {
			if owner.Kind == "ServiceClass" {
				found = true
				break
			}
		}
