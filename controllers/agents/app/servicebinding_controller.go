...
func (r *ServiceBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// If ServiceBinding is not valid do not reconcile it
	if ok, err := r.ensureServiceBindingIsValid(ctx, &serviceBinding); err != nil || !ok {
		return ctrl.Result{}, err
	}
...
func (r *ServiceBindingReconciler) ensureServiceBindingIsValid(ctx context.Context, sb *primazaiov1alpha1.ServiceBinding) (bool, error) {
	...
	c, s := func() (metav1.Condition, primazaiov1alpha1.ServiceBindingState) {
		if sb.Spec.Application.Name == "" && sb.Spec.Application.Selector == nil {
			return metav1.Condition{
				...
				Status:             metav1.ConditionFalse,
				Reason:             primazaiov1alpha1.ReasonMissingField,
				Message:            "One among application name and selector is needed",
		...
		return metav1.Condition{
			...
			Message:            "ServiceBinding is valid",
		}, primazaiov1alpha1.ServiceBindingStateReady
	}()

	l.Info("updating service binding status conditions")
	if err := r.updateServiceBindingStatus(ctx, sb, c, s); err != nil {
		l.Error(err, "unable to update service binding status conditions")
		return c.Status == metav1.ConditionTrue, err
	}

	l.Info("service binding status conditions updated")
